...
interface CreateTaskRunResponse {
  run_id: string;
  [key: string]: any;
}
...
async function createTaskRun(task: Task, env: Env) {
  ...
  const result: CreateTaskRunResponse = await response.json();
...
async function handleWebhook(request: Request, env: Env): Promise<Response> {
  try {
    ...
    if (webhook.type === 'task_run.status' && webhook.data?.status === 'completed') {
...
async function serveHomepage(env: Env): Promise<Response> {
  try {
    ...
    // Inject tasks data
    html = html.replace('</head>', `<script>window.tasks = ${JSON.stringify(tasksConfig.tasks)}</script></head>`);
