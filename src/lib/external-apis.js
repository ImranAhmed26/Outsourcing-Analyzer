...
exports.fetchDuckDuckGoData = fetchDuckDuckGoData;
exports.fetchWikipediaData = fetchWikipediaData;
exports.fetchClearbitLogo = fetchClearbitLogo;
exports.fetchCompanyData = fetchCompanyData;
exports.isRetryableError = isRetryableError;
exports.retryWithBackoff = retryWithBackoff;
exports.retryExternalApiCall = retryExternalApiCall;
 ...
 */
function createExternalApiError(apiName, message, statusCode = 500, details) {
    return {
        type: types_1.ErrorType.EXTERNAL_API_ERROR,
        apiName,
        message,
        statusCode,
        details,
 ...
 */
async function fetchDuckDuckGoData(companyName) {
    try {
        ...
        if (!response.ok) {
            throw createExternalApiError('DuckDuckGo', `DuckDuckGo API returned ${response.status}`, response.status, `Failed to fetch data for company: ${companyName}`);
        }
        ...
        if (error instanceof Error && error.message === 'Request timeout') {
            throw createExternalApiError('DuckDuckGo', 'DuckDuckGo API request timed out', 408, `Timeout after ${API_TIMEOUT}ms for company: ${companyName}`);
        }
        ...
        if (error && typeof error === 'object' && 'type' in error && error.type === types_1.ErrorType.EXTERNAL_API_ERROR) {
            throw error;
        }
        throw createExternalApiError('DuckDuckGo', 'Unexpected error fetching from DuckDuckGo API', 500, error instanceof Error ? error.message : 'Unknown error');
    }
}
/**
...
async function fetchWikipediaData(companyName) {
    try {
        ...
        if (!response.ok) {
            ...
            throw createExternalApiError('Wikipedia', `Wikipedia API returned ${response.status}`, response.status, `Failed to fetch data for company: ${companyName}`);
        }
        const data = await response.json();
        const companyData = {
            name: companyName,
        ...
        if (error instanceof Error && error.message === 'Request timeout') {
            throw createExternalApiError('Wikipedia', 'Wikipedia API request timed out', 408, `Timeout after ${API_TIMEOUT}ms for company: ${companyName}`);
        }
        if (error instanceof Error && error.name === 'TypeError') {
        ...
        throw createExternalApiError('Wikipedia', 'Unexpected error fetching from Wikipedia API', 500, error instanceof Error ? error.message : 'Unknown error');
    }
}
/**
 * Fetches company logo from Clearbit Logo API with fallback handling
 */
async function fetchClearbitLogo(companyName, website) {
    try {
        ...
        if (error instanceof Error && error.message === 'Request timeout') {
            throw createExternalApiError('Clearbit', 'Clearbit Logo API request timed out', 408, `Timeout after ${API_TIMEOUT}ms for company: ${companyName}`);
        }
        if (error instanceof Error && error.name === 'TypeError') {
 ...
 */
async function retryExternalApiCall(apiName, fn, maxRetries = 2) {
    ...
    }, maxRetries, 1000, // 1 second base delay
